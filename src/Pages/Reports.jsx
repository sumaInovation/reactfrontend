...
const Reports = () => {
  ...
  const [ReqDates, setrequierdataes] = useState([])
  ...
  const Labels = ['2023-12-01', '2023-12-02', '2023-12-03', '2023-12-04', '2023-12-05']
    ...
    <>
      <div className='mt-[80px] text-white grid grid-cols-1  lg:grid-cols-5 gap-6'>
        ...
        <button className={`m-3 text-white rounded-lg p-3 bg-green-600 w-[150px] lg:w-[200px]
          ...
          onClick={HandleHTTP}
          disabled={!inputValue} // Button is disabled if no value is entered
        >Search</button>
        <button className='m-3 text-white rounded-lg p-3 bg-green-600 w-[150px] lg:w-[200px]'
      ...
      {/* Formated Tables List */}

      {isDownload ? <div>
        <DownloadInvoice lbl={generateDateRange(StartTime, EndTime)} Items={selecteitems} piedata={piedatas} ldata={linedatas} />
      </div>


        : !isbutton ?//below part user not request download
          (<div>
          ...
          : (isloading ? (<div className='text-white grid lg:grid-cols-3 grid-cols-1   m-3'>
      ...
      <div className='h-[200px] text-white'>{selecteitems}</div>
    </>
  )
}
